...
void runIteratedMatrixMult(Tensor<float, 3, true> &c, bool transC,
                           ...
                           Tensor<BT, 3, true> &b, bool transB, float alpha,
                           float beta, cublasHandle_t handle,
                           cudaStream_t stream) {
  ...
  for (int i = 0; i < a.getSize(0); ++i) {
    ...
    runMatrixMult(cView, transC, aView, transA, bView, transB, alpha, beta,
                  handle, stream);
  }
}
...
void runIteratedMatrixMult(Tensor<float, 3, true> &c, bool transC,
                           Tensor<AT, 4, true> &a, bool transA,
                           Tensor<BT, 3, true> &b, bool transB, float alpha,
                           float beta, cublasHandle_t handle,
                           cudaStream_t stream) {
