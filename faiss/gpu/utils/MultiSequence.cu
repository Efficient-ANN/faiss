...
__device__ inline void toMultiIndex(int &codebookSize, IndexT &index1,
                                    IndexT &index2, MultiIndexT &multiIndex) {
  multiIndex = index1 + index2 * (MultiIndexT)codebookSize;
}

template <typename T, typename TVec2>
__device__ inline void
multiSequence_thread(float *heapValues, ushort2 *heapIds,
                     unsigned short *traversed, unsigned w, float *d1, T *i1,
                     float *d2, T *i2, float *dr, TVec2 *ir) {
  ...
  for (unsigned currentW = 1; currentW < w; currentW++) {
    ...
    if (traversed[i + 1] == j) {
      heap_push<unsigned short, ushort2>(heapSize, heapValues, heapIds,
                                         d1[i + 1] + d2[j], i + 1, j);
    }
...
__device__ inline void
multiSequence_thread(float *heapValues, ushort2 *heapIds,
                     unsigned short *traversed, unsigned w,
                     unsigned short inputLength, float *d1, T *i1, float *d2,
                     T *i2, float *dr, TVec2 *ir) {
...
__device__ inline void multiSequence_thread(float *heapValues, ushort2 *heapIds,
                                            unsigned short *traversed,
                                            unsigned w, float *d1, T *i1,
                                            float *d2, T *i2, float *dr,
                                            int codebookSize, MultiIndexT *ir) {
...
multiSequence_thread(float *heapValues, ushort2 *heapIds,
                     unsigned short *traversed, unsigned w,
                     unsigned short inputLength, float *d1, T *i1, float *d2,
                     T *i2, float *dr, int codebookSize, MultiIndexT *ir) {
...
__global__ void multiSequence_local(unsigned numThreadsGrid, unsigned w,
                                    float *d1, T *i1, float *d2, T *i2,
                                    float *dr, TVec2 *ir) {
  ...
  if (threadIdxGrid < numThreadsGrid) {
    ...
    multiSequence_thread<T, TVec2>(heapValues, heapIds, traversed, w, threadD1,
                                   threadI1, threadD2, threadI2, threadDr,
                                   threadIr);
  }
}
...
template <unsigned MaxInLength, typename T, typename TVec2>
__global__ void multiSequence_local(unsigned numThreadsGrid, unsigned w,
                                    unsigned short inputLength, float *d1,
                                    T *i1, float *d2, T *i2, float *dr,
                                    TVec2 *ir) {
  ...
  if (threadIdxGrid < numThreadsGrid) {
    ...
    multiSequence_thread<T, TVec2>(heapValues, heapIds, traversed, w,
                                   inputLength, threadD1, threadI1, threadD2,
                                   threadI2, threadDr, threadIr);
  }
}
...
__global__ void multiSequence_local(unsigned numThreadsGrid, unsigned w,
  ...
  if (threadIdxGrid < numThreadsGrid) {
    ...
    multiSequence_thread<T, MultiIndexT>(heapValues, heapIds, traversed, w,
                                         threadD1, threadI1, threadD2, threadI2,
                                         threadDr, codebookSize, threadIr);
  }
}
...
__global__ void multiSequence_local(unsigned numThreadsGrid, unsigned w,
  ...
  if (threadIdxGrid < numThreadsGrid) {
    ...
    multiSequence_thread<T, MultiIndexT>(
        heapValues, heapIds, traversed, w, inputLength, threadD1, threadI1,
        threadD2, threadI2, threadDr, codebookSize, threadIr);
  }
}

// performs much faster than local, but has memory limit
template <typename T, typename TVec2>
__global__ void multiSequence_shared(unsigned numThreadsGrid, unsigned w,
                                     float *d1, T *i1, float *d2, T *i2,
                                     float *dr, TVec2 *ir) {
  ...
  if (threadIdxGrid < numThreadsGrid) {
    ...
    unsigned short *traversed =
        (unsigned short
             *)&buffer[bufferStride + (sizeof(float) + sizeof(ushort2)) * w];
...
template <typename T, typename TVec2>
void multiSequenceLocalKernel(int numOfBlocks, int blockSize,
                              cudaStream_t stream, int numThreadsGrid, int w,
                              float *d1, T *i1, float *d2, T *i2, float *dr,
                              TVec2 *ir) {
#define RUN_MS(MaxInLength)                                                    \
  do {                                                                         \
    multiSequence_local<MaxInLength, T, TVec2>                                 \
        <<<numOfBlocks, blockSize, 0, stream>>>(numThreadsGrid, w, d1, i1, d2, \
                                                i2, dr, ir);                   \
  } while (0)
...
void multiSequenceLocalKernel(int numOfBlocks, int blockSize,
                              cudaStream_t stream, int numThreadsGrid, int w,
                              unsigned short inputLength, float *d1, T *i1,
                              float *d2, T *i2, float *dr, TVec2 *ir) {
#define RUN_MS(MaxInLength)                                                    \
  do {                                                                         \
    multiSequence_local<MaxInLength, T, TVec2>                                 \
        <<<numOfBlocks, blockSize, 0, stream>>>(                               \
            numThreadsGrid, w, inputLength, d1, i1, d2, i2, dr, ir);           \
  } while (0)
...
void multiSequenceLocalKernel(int numOfBlocks, int blockSize,
                              cudaStream_t stream, int numThreadsGrid, int w,
                              unsigned short inputLength, float *d1, T *i1,
                              float *d2, T *i2, float *dr, int codebookSize,
                              MultiIndexT *ir) {
#define RUN_MS(MaxInLength)                                                    \
  do {                                                                         \
    multiSequence_local<MaxInLength, T, MultiIndexT>                           \
        <<<numOfBlocks, blockSize, 0, stream>>>(numThreadsGrid, w,             \
                                                inputLength, d1, i1, d2, i2,   \
                                                dr, codebookSize, ir);         \
  } while (0)
...
void runMultiSequence2T(const int numQueries, const int inLength, const int w,
  ...
  if (w <= inLength) {
    chooseMultiSequence2Params<T, MultiIndexT>(w, useSharedMemory, blockSize);
  } else {
    chooseMultiSequence2Params<T, MultiIndexT>(inLength, useSharedMemory,
                                               blockSize);
  }
