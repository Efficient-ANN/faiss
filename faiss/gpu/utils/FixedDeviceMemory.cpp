...
FixedDeviceMemory::FixedDeviceMemory(GpuResources *res, int device,
                                     size_t allocSize, MemorySpace space)
    : res_(res), device_(device), alloc_(nullptr), allocSize_(allocSize),
      offset_(0) {
  if (allocSize_ == 0) {
    return;
  }
...
int FixedDeviceMemory::getDevice() const { return device_; }
