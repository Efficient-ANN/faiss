...
#define BLOCK_SELECT_DECL(TYPE, DIR, WARP_Q)                                   \
  extern void runBlockSelect_##TYPE##_##DIR##_##WARP_Q##_(                     \
      Tensor<TYPE, 2, true> &in, Tensor<TYPE, 2, true> &outK,                  \
      Tensor<int, 2, true> &outV, bool dir, int k, cudaStream_t stream);       \
                                                                               \
  extern void runBlockSelectPair_##TYPE##_##DIR##_##WARP_Q##_(                 \
      Tensor<TYPE, 2, true> &inK, Tensor<int, 2, true> &inV,                   \
      Tensor<TYPE, 2, true> &outK, Tensor<int, 2, true> &outV, bool dir,       \
...
#define BLOCK_SELECT_DECL_INDEX(TYPE, DIR, WARP_Q, INDEX_TYPE)                 \
  extern void runBlockSelect_##TYPE##_##DIR##_##WARP_Q##_##INDEX_TYPE##_(      \
      Tensor<TYPE, 2, true> &in, Tensor<TYPE, 2, true> &outK,                  \
      Tensor<INDEX_TYPE, 2, true> &outV, bool dir, int k,                      \
      cudaStream_t stream);                                                    \
                                                                               \
  extern void runBlockSelectPair_##TYPE##_##DIR##_##WARP_Q##_##INDEX_TYPE##_(  \
      Tensor<TYPE, 2, true> &inK, Tensor<INDEX_TYPE, 2, true> &inV,            \
      Tensor<TYPE, 2, true> &outK, Tensor<INDEX_TYPE, 2, true> &outV,          \
...
#define BLOCK_SELECT_IMPL_INDEX(TYPE, DIR, WARP_Q, THREAD_Q, INDEX_TYPE)       \
  void runBlockSelect_##TYPE##_##DIR##_##WARP_Q##_##INDEX_TYPE##_(             \
    ...
    auto vInit = std::numeric_limits<INDEX_TYPE>::max();                       \
                                                                               \
    blockSelect<TYPE, INDEX_TYPE, DIR, WARP_Q, THREAD_Q,                       \
  ...
  void runBlockSelectPair_##TYPE##_##DIR##_##WARP_Q##_##INDEX_TYPE##_(         \
    ...
    auto vInit = std::numeric_limits<INDEX_TYPE>::max();                       \
                                                                               \
    blockSelectPair<TYPE, INDEX_TYPE, DIR, WARP_Q, THREAD_Q,                   \
                    kBlockSelectNumThreads>                                    \
        <<<grid, block, 0, stream>>>(inK, inV, outK, outV, kInit, vInit, k);   \
    CUDA_TEST_ERROR();                                                         \
  }
...
#define BLOCK_SELECT_CALL_INDEX(TYPE, DIR, WARP_Q, INDEX_TYPE)                 \
  runBlockSelect_##TYPE##_##DIR##_##WARP_Q##_##INDEX_TYPE##_(in, outK, outV,   \
                                                             dir, k, stream)

#define BLOCK_SELECT_PAIR_CALL(TYPE, DIR, WARP_Q)                              \
  runBlockSelectPair_##TYPE##_##DIR##_##WARP_Q##_(inK, inV, outK, outV, dir,   \
                                                  k, stream)
