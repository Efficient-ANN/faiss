...
void testAdd(int d, int coarseCodebookSize, int numSubQuantizers,
             int bitsPerCode, int numOfTrainingVecs, bool verbose = false) {
  ...
  { // checking inverted lists
    ...
    for (int i = 0; i < numOfIndexVecsList.size(); i++) {
      ...
      imipqGpu.updateExpectedNumAddsPerList(numOfIndexVecsList[i],
                                            indexVecs.data());
      faiss::gpu::CudaEvent updateEnd(
          res.getResources()->getDefaultStreamCurrentDevice());
      updateEnd.cpuWaitOnEvent();
      ...
      std::vector<faiss::Index::idx_t> outLabels(numOfIndexVecsList[i]);
      std::vector<float> outDistances(numOfIndexVecsList[i]);
      multiIndex.search(numOfIndexVecsList[i], indexVecs.data(), 1,
                        outDistances.data(), outLabels.data());

      std::vector<float> residuals(numOfIndexVecsList[i] * d);
      multiIndex.compute_nearest_residual_n(numOfIndexVecsList[i],
                                            indexVecs.data(), residuals.data());

      int subCodebookSize = 1 << bitsPerCode;
      std::vector<faiss::IndexFlat> subCentroidsIndex(numSubQuantizers);
      ...
      for (int j = 0; j < subCentroidsIndex.size(); j++) {
        ...
        subCentroidsIndex[j].add(subCodebookSize,
                                 &subCentroids[j * subCodebookSize * subDim]);
      }
        ...
        for (int j = 0; j < numSubQuantizers; j++) {
          ...
          for (int k = 0; k < subCodebookSize; k++) {
            for (int l = 0; l < subDim; l++) {
              std::cout << " " << subCentroidsIndex[j].xb[k * subDim + l];
            }
            std::cout << std::endl;
          }
        }
      ...
      faiss::fvec_split(subResiduals.data(), numSubQuantizers, residuals.data(),
                        numOfIndexVecsList[i], subDim);
        ...
        for (int j = 0; j < numSubQuantizers; j++) {
          ...
          for (int k = 0; k < numOfIndexVecsList[i]; k++) {
            for (int l = 0; l < subDim; l++) {
              std::cout
                  << " "
                  << subResiduals[(j * numOfIndexVecsList[i] + k) * subDim + l];
            }
            std::cout << std::endl;
          }
        }
      ...
      std::vector<faiss::Index::idx_t> nearestSubCentroid(
          numSubQuantizers * numOfIndexVecsList[i]);
      std::vector<float> nearestSubCentroidDistance(numSubQuantizers *
                                                    numOfIndexVecsList[i]);
      for (int j = 0; j < numSubQuantizers; j++) {
        subCentroidsIndex[j].search(
            numOfIndexVecsList[i],
            &subResiduals[j * numOfIndexVecsList[i] * subDim], 1,
      ...
      if (verbose) {
        for (int j = 0; j < numSubQuantizers; j++) {
          ...
          for (int k = 0; k < numOfIndexVecsList[i]; k++) {
            std::cout << " "
                      << nearestSubCentroid[j * numOfIndexVecsList[i] + k];
          }
          std::cout << std::endl;
        }
        std::cout << std::endl;
        ...
        for (int j = 0; j < numSubQuantizers; j++) {
          ...
          for (int k = 0; k < numOfIndexVecsList[i]; k++) {
            std::cout
                << " "
                << nearestSubCentroidDistance[j * numOfIndexVecsList[i] + k];
          }
          std::cout << std::endl;
        }
        std::cout << std::endl;
      ...
      std::vector<std::vector<uint8_t>> invListCodes(coarseCodebookSize *
                                                     coarseCodebookSize);
      std::vector<std::vector<faiss::Index::idx_t>> invListIds(
        ...
        for (int j = 0; j < invListCodes.size(); j++) {
          ...
          for (int k = 0; k < imipqGpu.getListLength(j); k++) {
            ...
            for (int l = 0; l < numSubQuantizers; l++) {
              std::cout << " "
                        << (int)invListCodes[j][k * numSubQuantizers + l];
            }
            std::cout << std::endl;
          }
        }
      ...
      std::vector<int> expectedListLength(coarseCodebookSize *
                                          coarseCodebookSize);
      ...
      for (int j = 0; j < numOfIndexVecsList[i]; j++) {
        ...
        for (int k = 0; k < numSubQuantizers; k++) {
          ...
          EXPECT_EQ((int)invListCodes[listId][codeId],
                    (int)nearestSubCentroid[nearesSubCentroidId])
              << "Id: " << j << " currentListVec: " << currentListVec
...
void testPrecomputedCodes(int d, int coarseCodebookSize, int numSubQuantizers,
                          int bitsPerCode, int numOfTrainingVecs,
                          bool verbose = false) {
  FAISS_ASSERT(d % numSubQuantizers == 0);
  faiss::gpu::StandardGpuResources res;
  // res.noTempMemory();
    ...
    for (int i = 0; i < multiIndex->getNumCodebooks(); i++) {
      ...
      for (int j = 0; j < multiIndex->getCodebookSize(); j++) {
        for (int k = 0; k < multiIndex->getSubDim(); k++) {
          std::cout << " "
                    << centroids[(i * multiIndex->getCodebookSize() + j) *
  ...
  for (int i = 0; i < multiIndex->getNumCodebooks(); i++) {
    faiss::fvec_split(
        &coarseSubCentroids[i * multiIndex->getCodebookSize() *
                            multiIndex->getSubDim()],
        numSubQuantizersPerCodebook,
        &centroids[i * multiIndex->getCodebookSize() * multiIndex->getSubDim()],
        multiIndex->getCodebookSize(), subQuantizerDim);
  }
    ...
    for (int i = 0; i < numSubQuantizers; i++) {
      ...
      for (int j = 0; j < multiIndex->getCodebookSize(); j++) {
        for (int k = 0; k < subQuantizerDim; k++) {
          std::cout
              << " "
              << coarseSubCentroids[(i * multiIndex->getCodebookSize() + j) *
                                        subQuantizerDim +
                                    k];
        }
        std::cout << std::endl;
    ...
    for (int i = 0; i < numSubQuantizers; i++) {
      ...
      for (int j = 0; j < subCodebookSize; j++) {
        for (int k = 0; k < subQuantizerDim; k++) {
          std::cout
              << " "
              << subCentroids[(i * subCodebookSize + j) * subQuantizerDim + k];
        }
        std::cout << std::endl;
      }
    }
    ...
    for (int j = 0; j < numSubQuantizers; j++) {
      for (int k = 0; k < subCodebookSize; k++) {
        ...
        for (int l = 0; l < subQuantizerDim; l++) {
          int coarseSubId =
              (j * multiIndex->getCodebookSize() + i) * subQuantizerDim + l;
          int subId = (j * subCodebookSize + k) * subQuantizerDim + l;
          expectedPreComputedCodes[tableId] +=
              coarseSubCentroids[coarseSubId] * subCentroids[subId];
        }
        ...
        EXPECT_EQ(precomputedCodes[tableId], expectedPreComputedCodes[tableId]);
      }
    }
  }
}
...
void testSearchPrecomputedCodes(int d, int numSubQuantizers, int bitsPerCode,
                                bool verbose = false) {
  FAISS_ASSERT(d % numSubQuantizers == 0);
  ...
  for (int param1Idx = 0; param1Idx < coarseCodebookSizeList.size();
    ...
    if (verbose) {
      std::cout << "coarseCodebookSize: " << coarseCodebookSize << std::endl;
    ...
    for (int param2Idx = 0; param2Idx < searchKList.size(); param2Idx++) {
      ...
      std::vector<int> nprobeList = {1,
                                     coarseCodebookSize * coarseCodebookSize};

      for (int param3Idx = 0; param3Idx < nprobeList.size(); param3Idx++) {
        ...
        std::vector<std::pair<ushort, ushort>> outLabelsPair(numOfQueries *
                                                             nprobe);
        faiss::gpu::GpuMultiIndex2 *multiIndex = imipqGpu.getQuantizer();
        multiIndex->search_pair(numOfQueries, queries.data(), nprobe,
                                term1.data(), outLabelsPair.data());
        ...
        if (verbose) {
          ...
          for (int i = 0; i < numOfQueries; i++) {
            ...
            for (int j = 0; j < nprobe; j++) {
              std::cout << " (" << outLabelsPair[i * nprobe + j].first << ", "
          ...
          for (int i = 0; i < coarseCodebookSize; i++) {
            ...
            for (int j = 0; j < numSubQuantizers; j++) {
              ...
              for (int k = 0; k < subCodebookSize; k++) {
                std::cout
                    << " "
                    << term2[(i * numSubQuantizers + j) * subCodebookSize + k];
              }
              std::cout << std::endl;
            }
            std::cout << std::endl;
        ...
        if (verbose) {
          ...
          for (int i = 0; i < numSubQuantizers; i++) {
            ...
            for (int j = 0; j < subCodebookSize; j++) {
              for (int k = 0; k < subQuantizerDim; k++) {
                std::cout << " "
                          << subCentroids[(i * subCodebookSize + j) *
                                              subQuantizerDim +
                                          k];
              }
              std::cout << std::endl;
        ...
        std::vector<float> term3(numOfQueries * numSubQuantizers *
                                 subCodebookSize);
        for (int i = 0; i < numOfQueries; i++) {
          for (int j = 0; j < numSubQuantizers; j++) {
            for (int k = 0; k < subCodebookSize; k++) {
              int term3Idx = (i * numSubQuantizers + j) * subCodebookSize + k;
              term3[term3Idx] = 0;
              for (int l = 0; l < subQuantizerDim; l++) {
                int queryIdx = i * d + j * subQuantizerDim + l;
                int subIdx = (j * subCodebookSize + k) * subQuantizerDim + l;
                term3[term3Idx] += queries[queryIdx] * subCentroids[subIdx];
              }
              term3[term3Idx] *= -2;
            }
          }
          ...
          for (int i = 0; i < numOfQueries; i++) {
            ...
            for (int j = 0; j < numSubQuantizers; j++) {
              ...
              for (int k = 0; k < subCodebookSize; k++) {
                std::cout
                    << " "
                    << term3[(i * numSubQuantizers + j) * subCodebookSize + k];
              }
              std::cout << std::endl;
            }
          }
          ...
          for (int i = 0; i < numOfQueries; i++) {
            ...
            for (int j = 0; j < numSubQuantizers; j++) {
              ...
              for (int k = 0; k < subCodebookSize; k++) {
                std::cout
                    << " "
                    << indexTerm3[(i * numSubQuantizers + j) * subCodebookSize +
                                  k];
              }
              std::cout << std::endl;
            }
        ...
        if (verbose) {
          for (int i = 0; i < invListCodes.size(); i++) {
            ...
            for (int j = 0; j < imipqGpu.getListLength(i); j++) {
              ...
              for (int k = 0; k < numSubQuantizers; k++) {
                std::cout << " "
                          << (int)invListCodes[i][j * numSubQuantizers + k];
              }
              std::cout << std::endl;
            }
          }
        ...
        for (int i = 0; i < numOfQueries; i++) {
          ...
          for (int j = 0; j < nprobe; j++) {
            std::pair<ushort, ushort> listId2 = outLabelsPair[i * nprobe + j];
            ...
            for (int k = 0; k < numSubQuantizersPerCodebook; k++) {
              for (int l = 0; l < subCodebookSize; l++) {
                term23[k * subCodebookSize + l] =
                    term2[(listId2.first * numSubQuantizers + k) *
                              subCodebookSize +
                          l] +
                    term3[(i * numSubQuantizers + k) * subCodebookSize + l];

                if (verbose) {
                  std::cout << term2[(listId2.first * numSubQuantizers + k) *
                                         subCodebookSize +
                                     l];
                  std::cout
                      << " + "
                      << term3[(i * numSubQuantizers + k) * subCodebookSize +
                               l];
                  std::cout << " = " << term23[k * subCodebookSize + l]
                            << std::endl;
                }
              }
            }
            ...
            for (int k = numSubQuantizersPerCodebook;
                 k < 2 * numSubQuantizersPerCodebook; k++) {
              for (int l = 0; l < subCodebookSize; l++) {
                term23[k * subCodebookSize + l] =
                    term2[(listId2.second * numSubQuantizers + k) *
                              subCodebookSize +
                          l] +
                    term3[(i * numSubQuantizers + k) * subCodebookSize + l];

                if (verbose) {
                  std::cout << term2[(listId2.second * numSubQuantizers + k) *
                                         subCodebookSize +
                                     l];
                  std::cout
                      << " + "
            ...
            for (int k = 0; k < imipqGpu.getListLength(listId); k++) {
              float result = term1[i * nprobe + j];

              if (verbose) {
                std::cout << "Code: " << k << std::endl;
                std::cout << "Distance: " << term1[i * nprobe + j];
              }

              for (int l = 0; l < numSubQuantizers; l++) {
                int subIdx =
                    (int)invListCodes[listId][k * numSubQuantizers + l];
                result += term23[l * subCodebookSize + subIdx];

                if (verbose) {
                  std::cout << " + " << term23[l * subCodebookSize + subIdx];
                }
              }

              listOuts.push_back(std::make_pair(invListIds[listId][k], result));

              if (verbose) {
                std::cout << " = " << result << std::endl;
                std::cout << "Id: " << invListIds[listId][k] << std::endl;
              }
            }

            std::sort(listOuts.begin(), listOuts.end(), compareIndexEntry);
          ...
          std::sort(allListsOuts.begin(), allListsOuts.end(),
                    compareIndexEntry);
          for (int j = 0; j < searchK; j++) {
            if (j < allListsOuts.size()) {
              expectedOutLabels[i * searchK + j] = allListsOuts[j].first;
              expectedOutDistances[i * searchK + j] = allListsOuts[j].second;
            } else {
          ...
          if (verbose) {
            for (int j = 0; j < searchK; j++) {
              std::cout << expectedOutLabels[i * searchK + j] << ": "
                        << expectedOutDistances[i * searchK + j] << std::endl;
            }
            std::cout << std::endl;
          }
        }
        ...
        std::vector<faiss::Index::idx_t> outLabels(numOfQueries * searchK);
        imipqGpu.search(numOfQueries, queries.data(), searchK,
                        outDistances.data(), outLabels.data());
          ...
          for (int i = 0; i < numOfQueries; i++) {
            ...
            for (int j = 0; j < searchK; j++) {
              std::cout << outLabels[i * searchK + j] << ": "
                        << outDistances[i * searchK + j] << std::endl;
            }
          }
        }
        ...
        for (int i = 0; i < numOfQueries; i++) {
          for (int j = 0; j < searchK; j++) {
            if (expectedOutLabels[i * searchK + j] != -1) {
              EXPECT_EQ(expectedOutDistances[i * searchK + j],
                        outDistances[i * searchK + j]);
            } else {
              EXPECT_EQ(expectedOutLabels[i * searchK + j],
...
void testCopyFrom(int d, int nbitsCoarseQuantizer, int coarseCodebookSize,
                  ...
                  int numOfTrainingVecs, int numOfIndexVecs) {
  constexpr int M = 2;
  ...
  for (int i = 0; i < imipqGpu.nlist; i++) {
    std::vector<faiss::Index::idx_t> gpuIndices = imipqGpu.getListIndices(i);
    const faiss::Index::idx_t *cpuIndices = imipqCpu.invlists->get_ids(i);
...
TEST(TestGpuIndexIMIPQ, copyFrom) {
  ...
  for (int i = 0; i < dList.size(); i++) {
    int numTrainingVecs =
        std::max((1 << bitsPerCode), numCentroidsPerCodebookList[i]) * 39;
    testCopyFrom(dList[i], nbitsList[i], numCentroidsPerCodebookList[i],
                 numSubQuantizers, bitsPerCode, numTrainingVecs,
                 numOfIndexVecsList[i]);
  }
}
...
TEST(TestGpuIndexIMIPQ, copyTo) {
  ...
  for (int i = 0; i < dList.size(); i++) {
    int numTrainingVecs =
        std::max((1 << bitsPerCode), numCentroidsPerCodebookList[i]) * 39;
    testCopyTo(dList[i], numCentroidsPerCodebookList[i], numSubQuantizers,
               bitsPerCode, numTrainingVecs, numOfIndexVecsList[i]);
  }
}
