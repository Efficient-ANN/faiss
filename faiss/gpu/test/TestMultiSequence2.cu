...
template <typename T>
void testUnifiedIndex(int w, int numOfQueries,
                      unsigned short numCoarseDistances, int codebookSize) {
  constexpr int NUM_CODEBOOKS = 2;
  unsigned inputSize = (unsigned)numOfQueries * numCoarseDistances;
  unsigned outputSize = (unsigned)numOfQueries * w;
  std::mt19937 rng;
  ...
  dr = new float[outputSize];
  ir = new faiss::Index::idx_t[outputSize];

  faiss::gpu::fromDevice<float, 2>(outDistances, dr, stream);
  faiss::gpu::fromDevice<faiss::Index::idx_t, 2>(outIndices, ir, stream);
