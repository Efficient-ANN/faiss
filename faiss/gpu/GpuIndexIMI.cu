...
GpuIndexIMI::GpuIndexIMI(GpuResourcesProvider *provider, int dims,
                         int coarseCodebookSize, GpuIndexIMIConfig config)
    : GpuIndex(provider->getResources(), dims, faiss::MetricType::METRIC_L2, 0,
               config),
      nlist(coarseCodebookSize * coarseCodebookSize), nprobe(1),
      imiConfig_(config) {
  FAISS_THROW_IF_NOT_MSG(nlist > 0, "nlist must be > 0");
  quantizer = new GpuMultiIndex2(provider, dims, coarseCodebookSize,
                                 imiConfig_.multiIndexConfig);
}

GpuIndexIMI::GpuIndexIMI(std::shared_ptr<GpuResources> resources, int dims,
                         int coarseCodebookSize, GpuIndexIMIConfig config)
    : GpuIndex(resources, dims, faiss::MetricType::METRIC_L2, 0, config),
      nlist(coarseCodebookSize * coarseCodebookSize), nprobe(1),
      imiConfig_(config) {
  FAISS_THROW_IF_NOT_MSG(nlist > 0, "nlist must be > 0");
  quantizer = new GpuMultiIndex2(resources, dims, coarseCodebookSize,
                                 imiConfig_.multiIndexConfig);
}
...
int GpuIndexIMI::getNumProbes() const { return nprobe; }
...
void GpuIndexIMI::add_with_ids(Index::idx_t n, const float *x,
...
void GpuIndexIMI::addPaged_(int n, const float *x, const Index::idx_t *ids) {
  if (n > 0) {
    ...
    if (totalSize > kAddPageSize || n > kAddVecSize) {
      ...
      for (size_t i = 0; i < (size_t)n; i += tileSize) {
        ...
        addPage_(curNum, x + i * (size_t)this->d, ids ? ids + i : nullptr);
      }
    } else {
      addPage_(n, x, ids);
    }
...
void GpuIndexIMI::addPage_(int n, const float *x, const Index::idx_t *ids) {
  ...
  auto vecs = toDeviceTemporary<float, 2>(
      ...
      stream, {quantizer->getNumCodebooks() * (int)n, quantizer->getSubDim()});

  CudaEvent copyEnd(stream);

  if (ids) {
    auto indices = toDeviceTemporary<Index::idx_t, 1>(
        resources_.get(), imiConfig_.device, const_cast<Index::idx_t *>(ids),
        stream, {n});
...
void GpuIndexIMI::search(Index::idx_t n, const float *x, Index::idx_t k,
  ...
  auto outDistances = toDeviceTemporary<float, 2>(
      resources_.get(), imiConfig_.device, distances, stream, {(int)n, (int)k});

  auto outLabels = toDeviceTemporary<Index::idx_t, 2>(
      resources_.get(), imiConfig_.device, labels, stream, {(int)n, (int)k});
