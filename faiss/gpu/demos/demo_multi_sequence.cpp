...
template <typename T, typename TVec2>
inline void heap_push(T &size, float* values, TVec2 *ids, float val, T id1, T id2){
    size++;
    values--;
    ids--;
...
template <typename T, typename TVec2>
void multiSequence(unsigned w, float *d1, T *i1, float *d2, T *i2, float *dr, TVec2 *ir){
    float heapValues[w];
    TVec2 heapIds[w];
    ...
    for(unsigned currentW = 1; currentW < w; currentW++){
        ...
        if(traversed[i + 1] == j){
            heap_push<T, TVec2>(heapSize, heapValues, heapIds,  d1[i + 1] + d2[j], i + 1, j);
        }
        if(i == 0 || traversed[i - 1] > j + 1){
            heap_push<T, TVec2>(heapSize, heapValues, heapIds, d1[i] + d2[j + 1], i, j + 1);
        }
...
template <typename T, typename TVec2>
void multiSequence(T length, unsigned w, float *d1, T *i1, float *d2, T *i2, float *dr, TVec2 *ir){
...
template <typename T, typename TVec2>
float run(int w, int numOfQueries, unsigned short numCoarseDistances, bool isGpu, float *elapsedTimeKernel = nullptr) {
  ...
  for (unsigned i = 0; i < numOfQueries; i++) {
    std::sort(d1 + i * numCoarseDistances,
              d1 + i * numCoarseDistances + numCoarseDistances);
    std::sort(d2 + i * numCoarseDistances,
  ...
  for (unsigned i = 0; i < numOfQueries; i++) {
    std::random_shuffle(&i1[i * numCoarseDistances],
                        &i1[(i + 1) * numCoarseDistances]);
    std::random_shuffle(&i2[i * numCoarseDistances],
      ...
      for(unsigned i = 0; i < numOfQueries; i++){
          if(w <= numCoarseDistances){
              multiSequence<T, TVec2>(w, d1 + i * numCoarseDistances, i1 + i * numCoarseDistances,
                d2 + i * numCoarseDistances, i2 + i * numCoarseDistances, dr + i * w, ir + i * w);
          } else {
              multiSequence<T, TVec2>(numCoarseDistances, w, d1 + i * numCoarseDistances, i1 + i * numCoarseDistances,
                d2 + i * numCoarseDistances, i2 + i * numCoarseDistances, dr + i * w, ir + i * w);
          }
      }
...
int main(int argc, char **argv) {
  std::vector<int> wList = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048};
  ...
  for (int i = 0; i < wList.size(); i++) {
      ...
      for (int j = 0; j < numQueriesList.size(); j++) {
          ...
          elapsedTime = run<ushort, ushort2>(w, numQueries, k, true, &elapsedTimeKernel);
          std::cout << "    Elapsed time GPU: " << elapsedTime << std::endl;
          std::cout << "    Elapsed time Kernel: " << elapsedTimeKernel << std::endl;

          for (int numOfThreads = 1; numOfThreads <= 16; numOfThreads *= 2) {
            omp_set_num_threads(numOfThreads);
            elapseTimeCpu = run<ushort, ushort2>(w, numQueries, k, false, nullptr);
            std::cout << "    Num. of threads: " << numOfThreads << std::endl;
            std::cout << "      Elapsed time CPU: " << elapseTimeCpu << std::endl;
            std::cout << "      Speedup: " << elapseTimeCpu / elapsedTime << std::endl;
            std::cout << "      Speedup Kernel: " << elapseTimeCpu / elapsedTimeKernel  << std::endl;
          }
      }
  }
  return 0;
