...
if [ $SEARCH = 0 ]; then
    mpirun -np ${N_GPUS} ${DEMO_DIR}/demo_imipq_gpu_sift_m_mpi 960 ${N_CENTROIDS} 8 8 "" ${TRAIN_SIZE} "" ${BASE_SIZE} "" 0 "" ${Q_INIT} ${Q_END} ${N_INIT} ${N_END} ${K_INIT} ${K_END} ${IS_FLOAT} ${N_THREADS} ${N_GPUS} ${USE_SHARD} 0 1 ${MAX_MEM} ${RESULT_DIR}/coarse/coarse_${INDEX_POSFIX}.bin ${RESULT_DIR}/index/index_${INDEX_POSFIX}.bin ${SEARCH} 0 0 1 0 ${USE_PRECOMP} ${USE_IMI} ${USE_GPU} ${PRINT_GPU_MEM} 0 | tee ${RESULT_DIR}/outs/${OUT_NAME}.txt
else
   mpirun -np ${N_GPUS} ${PROF_SCRIPT_DIR}/nvprof_wrapper.sh ${RESULT_DIR}/outs/${OUT_NAME} ${DEMO_DIR}/demo_imipq_gpu_sift_m_mpi 960 ${N_CENTROIDS} 8 8 "" ${TRAIN_SIZE} "" ${BASE_SIZE} "" 0 "" ${Q_INIT} ${Q_END} ${N_INIT} ${N_END} ${K_INIT} ${K_END} ${IS_FLOAT} ${N_THREADS} ${N_GPUS} ${USE_SHARD} 0 1 ${MAX_MEM} ${RESULT_DIR}/coarse/coarse_${INDEX_POSFIX}.bin ${RESULT_DIR}/index/index_${INDEX_POSFIX}.bin ${SEARCH} 0 0 1 0 ${USE_PRECOMP} ${USE_IMI} ${USE_GPU} ${PRINT_GPU_MEM} 0 | tee ${RESULT_DIR}/outs/${OUT_NAME}.txt
fi
