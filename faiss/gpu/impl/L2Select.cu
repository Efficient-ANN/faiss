  ...
  if (endRow) {
    for (int row = rowStart; row < productDistances.getSize(0); ++row) {
      for (int col = threadIdx.x; col < productDistances.getSize(1);
           col += blockDim.x) {
        distance[0] =
            Math<T>::add(centroidDistances[col], productDistances[row][col]);
      ...
      // Reduce within the block
      threadMin[0] = blockReduceAll<Pair<T, unsigned short>,
                                    Min<Pair<T, unsigned short>>, false, false>(
...
#pragma unroll
    ...
    // Reduce within the block
    blockReduceAll<kRowsPerBlock, Pair<T, unsigned short>,
                   Min<Pair<T, unsigned short>>, false, false>(
        threadMin, Min<Pair<T, unsigned short>>(), blockMin);
...
__global__ void l2SelectMinK(Tensor<T, 2, true> productDistances,
                             Tensor<T, 1, true> centroidDistances,
                             Tensor<T, 2, true> outDistances,
                             Tensor<unsigned short, 2, true> outIndices, int k,
                             T initK) {
  // Each block handles a single row of the distances (results)
  constexpr int kNumWarps = ThreadsPerBlock / kWarpSize;
  ...
  BlockSelect<T, unsigned short, false, Comparator<T>, NumWarpQ, NumThreadQ,
              ThreadsPerBlock>
...
void runL2SelectMin(Tensor<float, 2, true> &productDistances,
                    ...
                    Tensor<unsigned short, 2, true> &outIndices, int k,
                    cudaStream_t stream) {
  runL2SelectMin<float>(productDistances, centroidDistances, outDistances,
                        outIndices, k, stream);
}
