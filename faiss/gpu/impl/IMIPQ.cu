...
IMIPQ::IMIPQ(GpuResources *resources, MultiIndex2 *quantizer,
             ...
             MemorySpace space)
    : IMIBase(resources, quantizer, interleavedLayout, indicesOptions, space),
...
size_t IMIPQ::calcVectorsEncodingMemorySpaceSize(int numVecs,
...
size_t IMIPQ::calcMemorySpaceSize(int numVecs, int numSubQuantizers,
                                  int bitsPerSubQuantizer,
                                  bool interleavedLayout,
...
void IMIPQ::appendVectors_(Tensor<float, 2, true> &vecs,
...
std::vector<uint8_t> IMIPQ::translateCodesToGpu_(std::vector<uint8_t> codes,
...
std::vector<uint8_t> IMIPQ::translateCodesFromGpu_(std::vector<uint8_t> codes,
                                                   size_t numVecs) const {
  if (!interleavedLayout_) {
    return codes;
  }
...
void IMIPQ::query(Tensor<float, 2, true> &queries, int nprobe, int k,
...
void IMIPQ::query_split(Tensor<float, 2, true> &queries, int nprobe, int k,
                        Tensor<float, 2, true> &outDistances,
                        Tensor<long, 2, true> &outIndices) {
...
void IMIPQ::runCalcTerm3(Tensor<float, 2, true> &queries, int &numQueries,
                         int &numSubQuantizersPerCodebook,
...
void IMIPQ::runCalcTerm3(Tensor<float, 2, true> &queries,
                         Tensor<float, 3, true> &term3) {
  int numQueries = queries.getSize(0) / quantizer_->getNumCodebooks();
  int numSubQuantizersPerCodebook =
      numSubQuantizers_ / quantizer_->getNumCodebooks();
