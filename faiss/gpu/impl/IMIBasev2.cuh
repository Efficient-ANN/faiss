...
class IMIBasev2 {
public:
  IMIBasev2(GpuResources *resources,
            /// We do not own this reference
            MultiIndex2 *quantizer, bool interleavedLayout,
  ...
  std::vector<Index::idx_t> getListIndices(int listId);
...
protected:
  ...
  translateCodesFromGpu_(std::vector<uint8_t> codes, size_t numVecs) const = 0;
