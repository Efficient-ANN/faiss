...
bool IMIPQv2::isSupportedPQCodeLength(int size) {
  switch (size) {
  case 1:
  case 2:
  case 3:
...
size_t IMIPQv2::calcVectorsEncodingMemorySpaceSize(int numVecs,
...
size_t IMIPQv2::calcIndicesMemorySpaceSize(int numVecs,
...
size_t IMIPQv2::calcMemorySpaceSize(int numVecs, int numSubQuantizers,
                                    int bitsPerSubQuantizer,
                                    bool interleavedLayout,
                                    IndicesOptions options) {
...
void IMIPQv2::appendVectors_(Tensor<float, 2, true> &vecs,
...

  {
    ...
    for (int subQ = 0; subQ < numSubQuantizers_; ++subQ) {
      ...
      runL2Distance(resources_, pqCentroidsMiddleCodeView,
                    ...
                    1, closestSubQDistanceView, closestSubQIndexView,
    ...
    // residual vector.
    auto closestSubQIndex8View =
        closestSubQIndex8.view<2>({numSubQuantizers_, residuals.getSize(0)});
  ...
  if (indicesOptions_ == INDICES_64_BIT) {
    DeviceTensor<Index::idx_t, 1, true> deviceListIndexTensor(
        (Index::idx_t *)deviceListIndices_.data(),
        {(int)(deviceListIndices_.size() / sizeof(Index::idx_t))});
    runIMIIndicesAppend(quantizer_->getCodebookSize(), listIds, listOffset,
                        indices, indicesOptions_, deviceListIndexTensor,
                        deviceListOffsetsTensor, stream);
  } else {
  ...
  runIMIPQAppend(quantizer_->getCodebookSize(), listIds, listOffset, encodings,
                 ...
                 numSubQuantizers_, stream);
}

size_t IMIPQv2::getGpuVectorsEncodingSize_(int numVecs) const {
  return calcVectorsEncodingMemorySpaceSize(
      numVecs, numSubQuantizers_, bitsPerSubQuantizer_, interleavedLayout_);
}

size_t IMIPQv2::getCpuVectorsEncodingSize_(int numVecs) const {
...
std::vector<uint8_t> IMIPQv2::translateCodesToGpu_(std::vector<uint8_t> codes,
                                                   size_t numVecs) const {
  if (!interleavedLayout_) {
    return codes;
  }

  auto up = unpackNonInterleaved(std::move(codes), numVecs, numSubQuantizers_,
                                 bitsPerSubQuantizer_);
  return packInterleaved(std::move(up), numVecs, numSubQuantizers_,
                         bitsPerSubQuantizer_);
}

// Conver the GPU layout to the CPU layout
std::vector<uint8_t> IMIPQv2::translateCodesFromGpu_(std::vector<uint8_t> codes,
                                                     size_t numVecs) const {
  if (!interleavedLayout_) {
    return codes;
  }

  auto up = unpackInterleaved(std::move(codes), numVecs, numSubQuantizers_,
                              bitsPerSubQuantizer_);
  return packNonInterleaved(std::move(up), numVecs, numSubQuantizers_,
                            bitsPerSubQuantizer_);
}

void IMIPQv2::setPQCentroids_(float *data) {
  auto stream = resources_->getDefaultStreamCurrentDevice();
  ...
  DeviceTensor<float, 3, true> pqDeviceTranspose(
      resources_, makeDevAlloc(AllocType::Quantizer, stream),
...

  {
    ...
    DeviceTensor<float, 3, true> pqDevice(
        resources_, makeTempAlloc(AllocType::Quantizer, stream), pqHost);
...
void IMIPQv2::precomputeCodesT_() {
  auto stream = resources_->getDefaultStreamCurrentDevice();
  //
  ...
  DeviceTensor<float, 3, true> coarsePQProduct(
      resources_, makeTempAlloc(AllocType::QuantizerPrecomputedCodes, stream),
      {numSubQuantizers_, quantizer_->getCodebookSize(),
       numSubQuantizerCodes_});
  {
    ...
    auto centroidView = coarseCentroids.template view<3>(
        {coarseCentroids.getSize(0), numSubQuantizersPerCodebook,
         dimPerSubQuantizer_});

    // This is only needed temporarily
    DeviceTensor<float, 3, true> centroidsTransposed(
        resources_, makeTempAlloc(AllocType::QuantizerPrecomputedCodes, stream),
        {numSubQuantizersPerCodebook, coarseCentroids.getSize(0),
         dimPerSubQuantizer_});
    ...
    auto centroidTransposedView = centroidsTransposed.template view<4>(
        {numSubQuantizersPerCodebook, quantizer_->getNumCodebooks(),
         quantizer_->getCodebookSize(), dimPerSubQuantizer_});

    runIteratedMatrixMult<false>(coarsePQProduct, false, centroidTransposedView,
                                 false, pqCentroidsMiddleCode_, true, 2.0f,
                                 0.0f, resources_->getBlasHandleCurrentDevice(),
                                 stream);
  }
  ...
  // along columns of inner product  (centroid id)(sub q * code id)
  {
    ...
    runL2Norm(pqCentroidsMiddleCodeView, true, subQuantizerNorms, true, stream);
...
void IMIPQv2::precomputeCodes_() {
  FAISS_ASSERT(!quantizer_->getUseFloat16());
  precomputeCodesT_();
}

void IMIPQv2::query(Tensor<float, 2, true> &queries, int nprobe, int k,
  ...
  FAISS_ASSERT(outDistances.getSize(0) * quantizer_->getNumCodebooks() ==
               queries.getSize(0));
  ...
  if (precomputedCodes_) {
    ...
    runPQNoPrecomputedCodes_(queries, coarseDistances, coarseIndices, k,
                             outDistances, outIndices);
  }
  ...
  if (indicesOptions_ == INDICES_CPU) {
    ...
    ivfOffsetToUserIndex(hostOutIndices.data(), numLists_,
                         hostOutIndices.getSize(0), hostOutIndices.getSize(1),
...
void IMIPQv2::query_split(Tensor<float, 2, true> &queries, int nprobe, int k,
                          Tensor<float, 2, true> &outDistances,
                          Tensor<long, 2, true> &outIndices) {
  FAISS_ASSERT(queries.getSize(1) % quantizer_->getNumCodebooks() == 0);
  FAISS_ASSERT(queries.getSize(1) / quantizer_->getNumCodebooks() ==
               quantizer_->getSubDim());
  auto stream = resources_->getDefaultStreamCurrentDevice();

  auto queriesView =
      queries.view<3>({queries.getSize(0), quantizer_->getNumCodebooks(),
                       quantizer_->getSubDim()});

  DeviceTensor<float, 3, true> queriesTranspose(
      resources_, makeTempAlloc(AllocType::QuerySplitted, stream),
...
Tensor<float, 3, true> IMIPQv2::getPrecomputedCodesVecFloat32() {
...
void IMIPQv2::runPQPrecomputedCodes_(
  ...
  runCalcTerm3(queries, numQueries, numSubQuantizersPerCodebook,
               term3Transposed);

  auto term3TransposedView = term3Transposed.view<4>(
      {numQueries, quantizer_->getNumCodebooks(), numSubQuantizersPerCodebook,
       numSubQuantizerCodes_});
  ...
  if (useFloat16LookupTables_) {
    auto precomputedCodeViewHalf = precomputedCodeHalf_.view<4>(
        {precomputedCodeHalf_.getSize(0), quantizer_->getNumCodebooks(),
         numSubQuantizersPerCodebook, precomputedCodeHalf_.getSize(2)});
    term3Half = convertTensorTemporary<float, half, 4>(resources_, stream,
                                                       term3TransposedView);
    ...
    auto precomputedCodeView = precomputedCode_.view<4>(
        {precomputedCode_.getSize(0), quantizer_->getNumCodebooks(),
         numSubQuantizersPerCodebook, precomputedCode_.getSize(2)});
...
void IMIPQv2::runPQNoPrecomputedCodesT_(
    Tensor<float, 2, true> &queries,
    DeviceTensor<float, 2, true> &coarseDistances,
    DeviceTensor<ushort2, 2, true> &coarseIndices, int k,
    Tensor<float, 2, true> &outDistances, Tensor<long, 2, true> &outIndices) {}

void IMIPQv2::runPQNoPrecomputedCodes_(
    Tensor<float, 2, true> &queries,
    DeviceTensor<float, 2, true> &coarseDistances,
    DeviceTensor<ushort2, 2, true> &coarseIndices, int k,
    Tensor<float, 2, true> &outDistances, Tensor<long, 2, true> &outIndices) {
  if (quantizer_->getUseFloat16()) {
    runPQNoPrecomputedCodesT_<half>(queries, coarseDistances, coarseIndices, k,
                                    outDistances, outIndices);
  } else {
    runPQNoPrecomputedCodesT_<float>(queries, coarseDistances, coarseIndices, k,
                                     outDistances, outIndices);
  }
}
