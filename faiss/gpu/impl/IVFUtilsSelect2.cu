...
template <typename IndexT, int ThreadsPerBlock, int NumWarpQ, int NumThreadQ,
          bool Dir>
__global__ void
pass2SelectLists(Tensor<float, 2, true> heapDistances,
                 Tensor<int, 2, true> heapIndices,
                 ...
                 Tensor<ushort2, 2, true> topQueryToCentroid, int k,
                 IndicesOptions opt, Tensor<float, 2, true> outDistances,
                 Tensor<Index::idx_t, 2, true> outIndices) {
  constexpr int kNumWarps = ThreadsPerBlock / kWarpSize;
  ...
  constexpr auto kInit = Dir ? kFloatMin : kFloatMax;
  BlockSelect<float, int, Dir, Comparator<float>, NumWarpQ, NumThreadQ,
              ThreadsPerBlock>
      heap(kInit, -1, smemK, smemV, k);
  ...
  for (int i = threadIdx.x; i < k; i += blockDim.x) {
    ...
    if (v != -1) {
      ...
      // We do this by binary search in the prefix sum list.
      int probe = binarySearchForBucket(prefixSumOffsets[queryId].data(),
                                        prefixSumOffsets.getSize(1), offset);
