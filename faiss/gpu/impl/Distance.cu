...
template <typename T>
void runDistance(bool computeL2, GpuResources *res,
                 Tensor<T, 2, true> &centroids, bool centroidsRowMajor,
                 Tensor<float, 1, true> *centroidNorms,
  ...
  // If we're querying against a 0 sized set, just return empty results
  if (centroids.numElements() == 0) {
    thrust::fill(thrust::cuda::par.on(defaultStream), outDistances.data(),
                 outDistances.end(), Limits<float>::getMax());

    thrust::fill(thrust::cuda::par.on(defaultStream), outIndices.data(),
                 outIndices.end(), std::numeric_limits<unsigned short>::max());
  ...
  if (computeL2 && !centroidNorms) {
    cNorms = DeviceTensor<float, 1, true>(
        res, makeTempAlloc(AllocType::Other, defaultStream), {numCentroids});
    runL2Norm(centroids, centroidsRowMajor, cNorms, true, defaultStream);
    centroidNorms = &cNorms;
  }
  ...
  //
  DeviceTensor<float, 1, true> queryNorms(
      res, makeTempAlloc(AllocType::Other, defaultStream), {(int)numQueries});
  ...
  int tileRows = 0;
  int tileCols = 0;
  chooseTileSize(numQueries, numCentroids, dim, sizeof(T),
                 res->getTempMemoryAvailableCurrentDevice(), tileRows,
                 tileCols);
  ...
  // Temporary output memory space we'll use
  DeviceTensor<float, 2, true> distanceBuf1(
      res, makeTempAlloc(AllocType::Other, defaultStream),
      {tileRows, tileCols});
  DeviceTensor<float, 2, true> distanceBuf2(
      res, makeTempAlloc(AllocType::Other, defaultStream),
      {tileRows, tileCols});
  DeviceTensor<float, 2, true> *distanceBufs[2] = {&distanceBuf1,
                                                   &distanceBuf2};
  ...
  DeviceTensor<float, 2, true> outDistanceBuf2(
      res, makeTempAlloc(AllocType::Other, defaultStream),
      {tileRows, numColTiles * k});
  DeviceTensor<float, 2, true> *outDistanceBufs[2] = {&outDistanceBuf1,
                                                      &outDistanceBuf2};
  ...
  for (int i = 0; i < numQueries; i += tileRows) {
    ...
    auto queryView = queries.narrow(queriesRowMajor ? 0 : 1, i, curQuerySize);
    ...
    for (int j = 0; j < numCentroids; j += tileCols) {
      ...
      runMatrixMult(distanceBufView,
                    ...
                    centroidsView,
                    centroidsRowMajor, // transposed MM if row major
                    computeL2 ? -2.0f : 1.0f, 0.0f,
                    res->getBlasHandleCurrentDevice(), streams[curStream]);
      ...
      if (computeL2) {
        ...
        if (tileCols == numCentroids) {
          // Write into the final output
          runL2SelectMin(distanceBufView, *centroidNorms, outDistanceView,
                         outIndexView, k, streams[curStream]);

          if (!ignoreOutDistances) {
            // expand (query id) to (query id, k) by duplicating along rows
            // top-k ||c||^2 - 2qc + ||q||^2 in the form (query id, k)
            runSumAlongRows(queryNormNiew, outDistanceView,
                            true, // L2 distances should not go below zero due
                                  // to roundoff error
                            streams[curStream]);
          }
          ...
          // Write into our intermediate output
          runL2SelectMin(distanceBufView, centroidNormsView,
                         outDistanceBufColView, outIndexBufColView, k,
                         streams[curStream]);

          if (!ignoreOutDistances) {
            // expand (query id) to (query id, k) by duplicating along rows
            // top-k ||c||^2 - 2qc + ||q||^2 in the form (query id, k)
            runSumAlongRows(queryNormNiew, outDistanceBufColView,
                            true, // L2 distances should not go below zero due
                                  // to roundoff error
                            streams[curStream]);
          }
        ...
        if (tileCols == numCentroids) {
          // Write into the final output
          runBlockSelect(distanceBufView, outDistanceView, outIndexView, true,
                         k, streams[curStream]);
        } else {
          // Write into the intermediate output
          runBlockSelect(distanceBufView, outDistanceBufColView,
                         outIndexBufColView, true, k, streams[curStream]);
        }
      }
    }
...
template <typename T>
void runL2Distance(GpuResources *res, Tensor<T, 2, true> &centroids,
                   bool centroidsRowMajor,
                   Tensor<float, 1, true> *centroidNorms,
                   Tensor<T, 2, true> &queries, bool queriesRowMajor, int k,
                   Tensor<float, 2, true> &outDistances,
