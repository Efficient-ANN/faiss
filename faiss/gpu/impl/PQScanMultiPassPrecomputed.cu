...
template <typename EncodeT, int EncodeBits, typename CodeDistanceT>
__global__ void
pqScanPrecomputedInterleaved(// (query id)(probe id)
                             Tensor<float, 2, true> precompTerm1,
...
pqScanPrecomputedInterleaved(// (query id)(probe id)
  ...
  auto vecsBase = listCodes.data() + listOffsets[listId];
...
template <int NumSubQuantizers, typename LookupT, typename LookupVecT>
__global__ void
pqScanPrecomputedMultiPass(Tensor<float, 2, true> precompTerm1,
  ...
  auto probeId = blockIdx.x;
  auto codesPerSubQuantizer = precompTerm2.getSize(3);
  auto precompTermSizePerCodebook = precompTerm2.getSize(2) * codesPerSubQuantizer;
  ...
  loadPrecomputedTerm<LookupT, LookupVecT>(term23 + precompTermSizePerCodebook,
...
pqScanPrecomputedMultiPass(Tensor<float, 2, true> precompTerm1,
  ...
  uint8_t* codeList = listCodes.data() + listOffsets[listId];
...
runMultiPassTile(GpuResources* res,
  ...
  // Calculate offset lengths, so we know where to write out
  // intermediate results
  runCalcListOffsets(res, coarseCodebookSize, topQueryToCentroid, listLengths,
    prefixSumOffsets, thrustMem, stream);
...
runMultiPassTile(GpuResources* res,
  ...
  // Calculate offset lengths, so we know where to write out
  // intermediate results
  runCalcListOffsets(res, coarseCodebookSize, topQueryToCentroid, listOffsets,
    prefixSumOffsets, thrustMem, stream);
...
void runPQScanMultiPassPrecomputed(
    ...
    Tensor<uint8_t, 1, true, long> &listCodes, Tensor<int, 1, true> &listIndices,
    IndicesOptions indicesOptions, int maxListLength, int k,
    // output
    Tensor<float, 2, true> &outDistances,
    // output
    Tensor<Index::idx_t, 2, true> &outIndices, GpuResources *res) {
  runPQScanMultiPassPrecomputed<int>(
      precompTerm1, precompTerm2, precompTerm3, coarseCodebookSize,
      topQueryToCentroid, useFloat16Lookup, interleavedCodeLayout,
      bitsPerSubQuantizer, numSubQuantizers, numSubQuantizerCodes, listOffsets,
      listCodes, listIndices, indicesOptions, maxListLength, k, outDistances,
      outIndices, res);
}
